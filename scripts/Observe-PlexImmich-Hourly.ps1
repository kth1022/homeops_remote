...
$extraAllowedPattern = ""
if ($ExtraAllowedContainers.Count) {
  $extraAllowedPattern = "|" + ($ExtraAllowedContainers -join "|")
}

$stamp = Get-Date -Format "yyyyMMddTHHmmss"
$summary = Join-Path $Root "reports\truenas-plex-immich-hourly-$stamp.summary.log"
...
function Invoke-Check {
  ...
  $out = Join-Path $Root ("reports\truenas-plex-immich-hourly-{0}-check-{1}.log" -f $stamp, $Index)
  $checkScript = Join-Path $Root ("logs\truenas-plex-immich-hourly-{0}-check-{1}.sh" -f $stamp, $Index)
  $script = @"
date -Iseconds
echo '===== containers ====='
...
docker ps --format '{{.Names}}' | while IFS= read -r name; do
  case "`$name" in
    ix-tailscale-*|ix-dockge-*|ix-plex-*|immich_server|immich_machine_learning|immich_redis|immich_postgres$extraAllowedPattern) ;;
    *) echo "`$name" ;;
  esac
done
echo '===== health ====='
...
"@
  ...
  node scripts/Invoke-TrueNasCronCommand.js --command-file $checkScript --description "codex plex immich hourly check $Index" --output $out --timeout-sec 180 | Out-Host
...
function Stop-Immich {
  $out = Join-Path $Root ("reports\truenas-plex-immich-hourly-{0}-immich-stop.log" -f $stamp)
  $stopScript = Join-Path $Root ("logs\truenas-plex-immich-hourly-{0}-immich-stop.sh" -f $stamp)
  $script = @"
date -Iseconds
...
"@
  ...
  node scripts/Invoke-TrueNasCronCommand.js --command-file $stopScript --description "codex stop immich after hourly observation failure" --output $out --timeout-sec 180 | Out-Host
  Write-Summary "Immich stop attempted after hourly observation failure. report=$out"
}

Write-Summary "Starting hourly Plex+Immich observation."

for ($i = 0; $i -le $CheckCount; $i += 1) {
  if (-not (Invoke-Check -Index $i)) {
    Write-Summary "Hourly Plex+Immich observation failed."
...
$durationHours = [math]::Round(($CheckCount * $IntervalSeconds) / 3600, 2)
Write-Summary "Hourly Plex+Immich observation passed for $durationHours hours."
exit 0
